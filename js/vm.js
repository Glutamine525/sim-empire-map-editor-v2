var $vm = new Vue({
  ...
  methods: {
    ...
    onSelectBuilding(index, indexPath) {
      ...
      newHolding.background =
        selectedBuilding.background_color || selectedBuilding.backgroundColor;
      // newHolding.borderColor = selectedBuilding.border_color || selectedBuilding.borderColor;
      newHolding.borderColor = 'var(--color-border-base)';
      newHolding.borderWidth = 1;
      newHolding.isPreview = true;
      newHolding.isProtection =
        $config.civilBuilding[$config.civil]['防'].indexOf(
          selectedBuilding.text
        ) > -1 &&
        $config.civilBuilding[$config.civil]['防护'].indexOf(
    ...
    onClickInsertSpecialBuilding() {
      ...
      let v = this.specialBuildingList.find(
        v => v.name === this.specialBuilding.name
      );
      if (v) {
        this.$message({
    ...
    onChangeSignFontSize() {
      let reminder =
        this.userSign[this.radioIndexUserSign].lines[
    ...
    onChangeRadioUserSign() {
      this.radioIndexUserSign = this.userSign.findIndex(
        v => v.name === this.radioLabelUserSign
      );
    ...
    onChangeRadioUserSignLine() {
      this.radioIndexUserSignLine =
        +this.radioLabelUserSignLine.substring(
          1,
          this.radioLabelUserSignLine.length - 1
        ) - 1;
      this.radioLabelUserSignElement = '第1个';
      this.radioIndexUserSignElement = 0;
    },
    insertUserSign() {
      this.$prompt('请输入水印名称', '提示', {
        ...
        inputValidator: value =>
          this.userSign.findIndex(v => v.name === value) === -1,
    ...
    updateUserSign() {
      this.$prompt('请输入水印名称', '提示', {
        ...
        .then(({ value }) => {
          ...
          let newSign = Object.assign(
            {},
            this.userSign[this.radioIndexUserSign]
          );
          newSign.name = value;
    ...
    deleteUserSignLine() {
      this.$confirm('是否确认删除该行？', '提示', {
        ...
        .then(() => {
          ...
          this.userSign[this.radioIndexUserSign].lines.splice(
            this.radioIndexUserSignLine,
            1
          );
          this.radioLabelUserSignLine = '第1行';
    ...
    deleteUserSignElement() {
      this.$confirm('是否确认删除该元素？', '提示', {
        ...
        .then(() => {
          ...
          this.userSign[this.radioIndexUserSign].lines[
            this.radioIndexUserSignLine
          ].elements.splice(this.radioIndexUserSignElement, 1);
          this.radioLabelUserSignElement = '第1个';
          this.radioIndexUserSignElement = 0;
          this.$message({
            type: 'success',
    ...
    updateCivilEditorStep(step) {
      ...
      if (this.civilEditorActiveStep === 10) {
        if (
          this.civilEditorResicenceReq.length !==
          this.civilEditorResult['住宅'].length
        ) {
    ...
    insertCivilBuilding() {
      this.$prompt('请输入建筑名称', '提示', {
        ...
        .then(({ value }) => {
          ...
          let maxLength =
            BuildingColor[this.civilEditorStepLabel[this.civilEditorActiveStep]]
              .length;
          ...
          this.civilEditorResult[
            ...
            text: value,
            width:
              this.civilEditorStepLabel[this.civilEditorActiveStep] === '商业'
                ? 1
                : 2,
            height:
              this.civilEditorStepLabel[this.civilEditorActiveStep] === '商业'
            ...
            borderColor: 'var(--color-border-base)',
            isDecoration:
              this.civilEditorStepLabel[this.civilEditorActiveStep] === '美化',
            isMiracle:
              this.civilEditorStepLabel[this.civilEditorActiveStep] === '奇迹',
            isProtection: false,
            effect: 'plain',
            id: `${
              this.civilEditorStepLabel[this.civilEditorActiveStep]
    ...
    selectCivilTag(index, catagoryIndex) {
      if (catagoryIndex !== undefined) {
        ...
        this.civilEditorPreview =
          this.civilEditorResult[this.civilEditorStepLabel[catagoryIndex]][
            index
          ];
        this.civilEditorResult[this.civilEditorStepLabel[catagoryIndex]][
          index
      ...
      for (let v of this.civilEditorResult[
        this.civilEditorStepLabel[this.civilEditorActiveStep]
      ]) {
        v.effect = 'plain';
      }
